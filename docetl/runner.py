...
class DSLRunner(ConfigWrapper):
    ...
    def load_run_save(self) -> float:
        ...
        return self.total_cost, self.output_data
    ...
    def save(self, data: list[dict]) -> None:
        ...
        if output_config["type"] == "file":
            ...
            if output_config["path"].lower().endswith(".json"):
                with open(output_config["path"], "w", encoding="utf-8") as file:
                    json.dump(data, file, indent=2, ensure_ascii=False)
            else:  # CSV
                import csv

                with open(output_config["path"], "w", newline="", encoding="utf-8") as file:
        ...
        elif output_config["type"] == "list":
            self.console.log(
                f"[green]✓[/green] Final Output:\n{json.dumps(data, indent=2, ensure_ascii=False)}\n"
            )
        else:
            raise ValueError(
